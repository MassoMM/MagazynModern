...
{
    public partial class FormWW : Form
    {
        public FormWW()
        {
            InitializeComponent();
        ...
        private void FormWW_Load(object sender, EventArgs e)
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["newConnectionString"].ConnectionString);
            con.Open();
            // TODO: Ten wiersz kodu wczytuje dane do tabeli 'mydbDataSet.wwtbl' . Możesz go przenieść lub usunąć.
            this.wwtblTableAdapter.Fill(this.mydbDataSet.wwtbl);
            con.Close();
            LoadTheme();
            Edit(false);
        }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(txtSearch.Text))
            {
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["newConnectionString"].ConnectionString);
                con.Open();
                wwtblBindingSource.Filter = string.Format("kolor = '{0}' OR kod LIKE '*{1}*' OR ilosc = '{2}' OR uwagi LIKE '*{3}*' OR system = '{4}' ", txtkolekcja.Text, txtkanina.Text, txtbelka.Text, txtodpad.Text, txtuwagi.Text);
                con.Close();
            }
            else
            {
                wwtblBindingSource.Filter = string.Empty;
            }
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["newConnectionString"].ConnectionString);
                con.Open();
                mydbDataSet.wwtbl.AddwwtblRow(mydbDataSet.wwtbl.NewwwtblRow());
                wwtblBindingSource.MoveLast();
                txtkolekcja.Focus();
                con.Close();
            }
            catch (Exception ex)
            {
                ...
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["newConnectionString"].ConnectionString);
                con.Open();
                mydbDataSet.wwtbl.RejectChanges();
                con.Close();
            }
        }
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                wwtblBindingSource.RemoveCurrent();
            }
        }
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["newConnectionString"].ConnectionString);
                con.Open();
                wwtblBindingSource.EndEdit();
                wwtblTableAdapter.Update(mydbDataSet.wwtbl);
        ...
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["newConnectionString"].ConnectionString);
            string sqlcmd = "SELECT * FROM wwtbl WHERE kod LIKE '" + txtSearch.Text + "%'";
            con.Open();
